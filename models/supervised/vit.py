...
def find_png_files_in_folders(folder):
    png_files = []
...
def get_verification_dataset(generate_verification_dataset):
    ...
    if generate_verification_dataset == True:
        ...
        save_path = "data/AND/lightly-transformed-64/supervised/verification-dataset/05-06-2024-040222" #(10%) 05-06-2024-040222  (100%) #05-04-2024-221654
        train_verification_path = os.path.join(save_path, "train.parquet")
        test_verification_path = os.path.join(save_path, "test.parquet")
    ...
    def __init__(self,transform=None,should_invert=None, is_train=True):
        ...
        if is_train == True:
            self.img_pairs = pd.read_parquet(train_path)
             ...
             self.img_pairs = pd.read_parquet(test_path)#.sample(1000)
...
class SiameseNetwork(pl.LightningModule):
    def __init__(self, margin, learning_rate):
        super().__init__()
    ...
    def forward_once(self, x):
        output = self.backbone(x).flatten(start_dim=1)
